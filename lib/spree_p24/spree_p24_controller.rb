module SpreeP24Controller
  ...
  module InstanceMethods
    ...
    def payment_verify(response_params)
      return data = { :amount => 100.0, :crc_key => SpreeP24.crc_key }
    end
    ...
    def przelewy24_verify(params,ip)
      return '' unless SpreeP24.check_ip(ip)
      require 'net/https'
      require 'net/http'
      require 'open-uri'
      require 'openssl'
      ...
      params_new = {
        ...
        :p24_amount => SpreeP24.make_p24_amount(data[:amount]),
        :p24_currency => params[:p24_currency],
        :p24_order_id => params[:p24_order_id]
      }
      params_new[:p24_sign] = SpreeP24.verify_sign(data,params_new)

      url = URI.parse(SpreeP24.verification_request_url)
      ...
      return  SpreeP24.parse_response response.body
    end
